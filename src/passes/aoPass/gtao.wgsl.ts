...
const NUM_DIRECTIONS: u32 = 32u;
const NUM_STEPS: u32 = 16u;
const RADIUS: f32 = 2.0; // in world space

const DIRECTION_OFFSET = 0.0;
// const STEP_OFFSET = 0.0;

// const ENABLE_FALLOFF = 1; // TODO remove
const FALLOFF_START2: f32 = 0.16;
const FALLOFF_END2: f32 = 4.0;

// const ENABLE_ROTATION_JITTER = 1;
// const ENABLE_RADIUS_JITTER = 1;
// const ENABLE_COLOR_BOUNCE = 1;
// const JITTER_TYPE = 0;
...
fn gtao(
  ...
  let normalVS: vec3f = (viewMatrix * vec4f(normalWS, 1.)).xyz;
  // calculation uses left handed system
  // normalVS.z = - normalVS.z; // TODO?
  
  let toCamera: vec3f	= normalize(-positionVS.xyz);
  
  // test for background
  if (positionVS.z >= (0.999 * Z_FAR)) { return 0.0; } // close enough
  ...
  // var radius = RADIUS * (clipInfoZ / positionVS.z);
  // radius = max(f32(NUM_STEPS), radius);
  let radius = max(f32(NUM_STEPS), RADIUS * (positionVS.z / Z_FAR));
  // return radius / (f32(NUM_STEPS));
  
  let stepSize = radius / f32(NUM_STEPS);
  // var phi = 0.0; // (DIRECTION_OFFSET + noises.x) * PI;
  var ao = 0.0;
  // let division	= noises.y * stepSize;
  // let currStep	= 1.0 + division + 0.25 * stepSize * STEP_OFFSET;
  ...
  for (var i = 0u; i < NUM_DIRECTIONS; i++) {
    var phi = f32(i) * (PI / f32(NUM_DIRECTIONS)) + DIRECTION_OFFSET * PI;
    phi += rotJitterOffset;
...
fn gtaoFalloff(dist: f32) -> f32 {
  return 2.0 * saturate(
    (dist - FALLOFF_START2) / (FALLOFF_END2 - FALLOFF_START2)
  );
}
